...
Importante (para não sobrescrever o que o dono edita pelo painel):
  - Uma configuração só é sobrescrita quando a env var correspondente está
    definida E não-vazia. Se a env var não existir, o valor atual no banco
    (inclusive edições feitas pelo painel) é preservado.
  - A senha do admin só é redefinida se ADMIN_PASSWORD estiver definida.
